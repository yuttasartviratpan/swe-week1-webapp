...
def vending_machine_create(
    ...
    if request.method == "POST":
        ...
        if name and location:
            ...
            return JsonResponse(
                {"name": vending_machine.name, "location": vending_machine.location}
            )
        else:
...
def check_item_timeline(
    ...
    for index in range(len(date_list)):
        json_return_list.append(
            {"date": date_list[index], "quantity": item_quantity[index]}
        )
...
def check_stock_timeline(vending_machine_id: int) -> Union[JsonResponse, Http404]:
    ...
    time_stamp_list = []
    for time in time_stamp:
        time_stamp_list.append({"date": time.key, "items": time.items})
